...
import { doesExist, writeFile, nodeFsBackend } from '../../utils';
...
export async function createPropertiesI18nEntries(
    ...
    if (!(await doesExist(i18nFilePath, backend))) {
        await createNewI18nFile(i18nFilePath, root, backend);
    }
    return await writeToExistingI18nPropertiesFile(i18nFilePath, newI18nEntries, [], backend);
}
 ...
 * @param backend - Storage backend to use. Defaults to Node.js `fs/promises`.
 */
async function createNewI18nFile(
    i18nFilePath: string,
    root?: string,
    backend: StorageBackend = nodeFsBackend
): Promise<void> {
    ...
    return writeFile(i18nFilePath, content, backend);
}
